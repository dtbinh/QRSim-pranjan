classdef TaskEscape<Task
    ...
    methods (Sealed,Access=public)
        ...
        function UU = step(obj,U)
            ...
            [obsx,obsy,obsz] = obj.simState.platforms{1}.obstacleCheck();
            
            if(obj.simState.platforms{1}.obstacleCheck())
                global pid;
                global wp;
                global k;
                %pid = WaypointPID(obj.simState.DT);
                %wp = [0;0;0];
            
                if(~((obsx - obj.simState.platforms{1}.getX(1))<1))
                    wp(k,1) = obsx;
                end
                if(~((obsy - obj.simState.platforms{1}.getX(2))<1))
                    wp(k,2) = obsy;
                end
                if(~((obsz - obj.simState.platforms{1}.getX(3))<1))
                    wp(k,3) = obsz;
                end
             
                 UU = pid.computeU(obj.simState.platforms{1}.getX(),wp(k,:)',0);
            else
                 UU = U; 
            end
