classdef Pelican<Steppable & Platform
    ...
    properties (Access = public)
        ...
        elasticity = 0.2;     % pranjan. Explanation coming soon....
    end
    ...
    methods (Access = public)
        ...
        function outputArg = send_message(obj, msg, transmitter, dst)
            ...
            timestamp = obj.simState.platforms{dst}.peer_contact_time(msg.src);
            res = 0;
            if msg.src ~= dst && transmitter ~= dst && msg.timestamp > timestamp 
                ...
                D = D * obj.simState.dist_scale; % TODO: Scale D            
        ...
        function out = read_message(obj, uav_no)
            ...
            for i=1:nrows
                ...
                if msg.type == 2  % Coordinate update
                    % TODO: check if this message is later than the one already
                    % in there.
        ...
        function ob = PlumeDetect(obj, j)
            ...
            if(sqrt(((xp-x)*(xp-x)) + ((yp-y)*(yp-y)) + ((zp+z)*(zp+z)))< r)  % why is zp + z instead of zp - z?
                ...
                for k=1:length(obj.simState.platforms)
                    if k ~= j
                        obj.send_message(msg, j, k);  % For each other platform call send message
                    end
                end
